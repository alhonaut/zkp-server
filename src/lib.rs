use num_bigint::{BigUint, RandBigInt};
use rand::Rng;

pub struct ZKP {
    pub p: BigUint,
    pub q: BigUint,
    pub alpha: BigUint,
    pub beta: BigUint,
}

impl ZKP {
    // output y1 = n ^ exp mod p
    pub fn calculate_two(&self, exp: &BigUint) -> (BigUint, BigUint) {
        // n.modpow(exp, p)
        let result1 = self.alpha.modpow(exp, &self.p);
        let result2 = self.beta.modpow(exp, &self.p);

        (result1, result2)
    }

    // output s = k - x * c mod q
    pub fn solve(&self, k: &BigUint, c: &BigUint, x: &BigUint) -> BigUint {
        if *k >= x * c {
            return (k - x * c).modpow(&BigUint::from(1u32), &self.q);
        }
        &self.q - (c * x - k).modpow(&BigUint::from(1u32), &self.q)
    }

    // r1 = aplha^s - y1*c
    // r2 = beta^s - y2*c
    pub fn verify(
        &self,
        r1: &BigUint,
        r2: &BigUint,
        y1: &BigUint,
        y2: &BigUint,
        c: &BigUint,
        s: &BigUint,
    ) -> bool {
        *r1 == (&self.alpha.modpow(s, &self.p) * y1.modpow(c, &self.p))
            .modpow(&BigUint::from(1u32), &self.p)
            && *r2
                == (&self.beta.modpow(s, &self.p) * y2.modpow(c, &self.p))
                    .modpow(&BigUint::from(1u32), &self.p)
    }

    pub fn generate_random_number(max_limit: &BigUint) -> BigUint {
        let mut rng = rand::thread_rng();

        rng.gen_biguint_below(max_limit)
    }

    pub fn generate_random_string(size: usize) -> String {
        rand::thread_rng()
            .sample_iter(rand::distributions::Alphanumeric)
            .take(size)
            .map(char::from)
            .collect()
    }

    pub fn get_constants() -> (BigUint, BigUint, BigUint, BigUint) {
        let p = BigUint::from_bytes_be(&hex::decode("").unwrap());
        let q = BigUint::from_bytes_be(
            &hex::decode("F518AA8781A8DF278ABA4E7D64B7CB9D49462353").unwrap(),
        );

        let alpha = BigUint::from_bytes_be(
            &hex::decode("").unwrap(),
        );

        // beta = alpha^i is also a generator
        let exp = BigUint::from_bytes_be(&hex::decode("266FEA1E5C41564B777E69").unwrap());
        let beta = alpha.modpow(&exp, &p);

        (p, q, alpha, beta)
    }
}

#[cfg(test)]
mod test {
    // its other space so we need to include all func outside with super::*
    use super::*;

    #[test]
    fn test_toy_exmp() {
        let alpha = BigUint::from(4u32);
        let beta = BigUint::from(9u32);
        let p = BigUint::from(23u32); // prime number
        let q = BigUint::from(11u32); // order how much elements in a group
        let zkp = ZKP {
            p: p.clone(),
            q: q.clone(),
            alpha: alpha.clone(),
            beta: beta.clone(),
        };

        let x = BigUint::from(6u32); // secret
        let k = BigUint::from(7u32); // key

        let c = BigUint::from(4u32); // rand value, here is not value but in production will be

        let (y1, y2) = zkp.calculate_two(&x);
        assert_eq!(y1, BigUint::from(2u32));
        assert_eq!(y2, BigUint::from(3u32));

        let (r1, r2) = zkp.calculate_two(&k);
        //let r2 = ZKP::calculate_two(&beta, &k, &p);
        assert_eq!(r1, BigUint::from(8u32));
        assert_eq!(r2, BigUint::from(4u32));

        let s = zkp.solve(&k, &c, &x);
        assert_eq!(s, BigUint::from(5u32));

        let result = zkp.verify(&r1, &r2, &y1, &y2, &c, &s);
        assert!(result);
    }

    #[test]
    fn test_toy_exmp_rand() {
        let alpha = BigUint::from(4u32);
        let beta = BigUint::from(9u32);
        let p = BigUint::from(23u32); // prime number
        let q = BigUint::from(11u32); // order how much elements in a group
        let zkp = ZKP {
            p: p.clone(),
            q: q.clone(),
            alpha: alpha.clone(),
            beta: beta.clone(),
        };

        let x = BigUint::from(6u32); // secret
        let k = ZKP::generate_random_number(&q); // key, need to be rand

        let c = ZKP::generate_random_number(&q); // rand value, here is not value but in production will be

        let (y1, y2) = zkp.calculate_two(&x);
        let (r1, r2) = zkp.calculate_two(&k);

        let s = zkp.solve(&k, &c, &x);

        let result = zkp.verify(&r1, &r2, &y1, &y2, &c, &s);
        assert!(result);
    }

    #[test]
    fn test_1024_bit_const() {
        //
        //    Reference: https://www.rfc-editor.org/rfc/rfc5114#page-15
        //
        //    The hexadecimal value of the prime is:
        //
        //    p = B10B8F96 A080E01D DE92DE5E AE5D54EC 52C99FBC FB06A3C6
        //        9A6A9DCA 52D23B61 6073E286 75A23D18 9838EF1E 2EE652C0
        //        13ECB4AE A9061123 24975C3C D49B83BF ACCBDD7D 90C4BD70
        //        98488E9C 219A7372 4EFFD6FA E5644738 FAA31A4F F55BCCC0
        //        A151AF5F 0DC8B4BD 45BF37DF 365C1A65 E68CFDA7 6D4DA708
        //        DF1FB2BC 2E4A4371
        //
        //    The hexadecimal value of the generator is:
        //
        //    g(alpha) = A4D1CBD5 C3FD3412 6765A442 EFB99905 F8104DD2 58AC507F
        //        D6406CFF 14266D31 266FEA1E 5C41564B 777E690F 5504F213
        //        160217B4 B01B886A 5E91547F 9E2749F4 D7FBD7D3 B9A92EE1
        //        909D0D22 63F80A76 A6A24C08 7A091F53 1DBF0A01 69B6A28A
        //        D662A4D1 8E73AFA3 2D779D59 18D08BC8 858F4DCE F97C2A24
        //        855E6EEB 22B3B2E5
        //
        //    The generator generates a prime-order subgroup of size:
        //    q = F518AA87 81A8DF27 8ABA4E7D 64B7CB9D 49462353
        //
        let p = BigUint::from_bytes_be(&hex::decode("").unwrap()); // prime number
        let q = BigUint::from_bytes_be(
            &hex::decode("F518AA8781A8DF278ABA4E7D64B7CB9D49462353").unwrap(),
        ); // order how much elements in a group

        // alpha is a generator
        let alpha = BigUint::from_bytes_be(
            &hex::decode("").unwrap(),
        );
        // beta = alpha^i is a generator too
        let beta = alpha.modpow(&ZKP::generate_random_number(&q), &p);
        let zkp = ZKP {
            p: p.clone(),
            q: q.clone(),
            alpha: alpha.clone(),
            beta: beta.clone(),
        };

        let x = ZKP::generate_random_number(&q); // secret
        let k = ZKP::generate_random_number(&q); // key, need to be rand

        let c = ZKP::generate_random_number(&q); // rand value, here is not value but in production will be

        let (y1, y2) = zkp.calculate_two(&x);
        let (r1, r2) = zkp.calculate_two(&k);

        let s = zkp.solve(&k, &c, &x);

        let result = zkp.verify(&r1, &r2, &y1, &y2, &c, &s);
        assert!(result);
    }

    #[test]
    fn test_2048_bit_const() {
        //    Now use 2048-bit constants
        //
        //    Reference: https://www.rfc-editor.org/rfc/rfc5114#page-15
        //
        //    The hexadecimal value of the prime is:
        //
        //    p = AD107E1E 9123A9D0 D660FAA7 9559C51F A20D64E5 683B9FD1
        //        B54B1597 B61D0A75 E6FA141D F95A56DB AF9A3C40 7BA1DF15
        //        EB3D688A 309C180E 1DE6B85A 1274A0A6 6D3F8152 AD6AC212
        //        9037C9ED EFDA4DF8 D91E8FEF 55B7394B 7AD5B7D0 B6C12207
        //        C9F98D11 ED34DBF6 C6BA0B2C 8BBC27BE 6A00E0A0 B9C49708
        //        B3BF8A31 70918836 81286130 BC8985DB 1602E714 415D9330
        //        278273C7 DE31EFDC 7310F712 1FD5A074 15987D9A DC0A486D
        //        CDF93ACC 44328387 315D75E1 98C641A4 80CD86A1 B9E587E8
        //        BE60E69C C928B2B9 C52172E4 13042E9B 23F10B0E 16E79763
        //        C9B53DCF 4BA80A29 E3FB73C1 6B8E75B9 7EF363E2 FFA31F71
        //        CF9DE538 4E71B81C 0AC4DFFE 0C10E64F
        //
        //    The hexadecimal value of the generator is:
        //
        //    g(alpha) = AC4032EF 4F2D9AE3 9DF30B5C 8FFDAC50 6CDEBE7B 89998CAF
        //        74866A08 CFE4FFE3 A6824A4E 10B9A6F0 DD921F01 A70C4AFA
        //        AB739D77 00C29F52 C57DB17C 620A8652 BE5E9001 A8D66AD7
        //        C1766910 1999024A F4D02727 5AC1348B B8A762D0 521BC98A
        //        E2471504 22EA1ED4 09939D54 DA7460CD B5F6C6B2 50717CBE
        //        F180EB34 118E98D1 19529A45 D6F83456 6E3025E3 16A330EF
        //        BB77A86F 0C1AB15B 051AE3D4 28C8F8AC B70A8137 150B8EEB
        //        10E183ED D19963DD D9E263E4 770589EF 6AA21E7F 5F2FF381
        //        B539CCE3 409D13CD 566AFBB4 8D6C0191 81E1BCFE 94B30269
        //        EDFE72FE 9B6AA4BD 7B5A0F1C 71CFFF4C 19C418E1 F6EC0179
        //        81BC087F 2A7065B3 84B890D3 191F2BFA
        //
        //    The generator generates a prime-order subgroup of size:
        //    q = 801C0D34 C58D93FE 99717710 1F80535A 4738CEBC BF389A99
        //        B36371EB
        //
        let p = BigUint::from_bytes_be(&hex::decode("").unwrap());
        let q = BigUint::from_bytes_be(
            &hex::decode("801C0D34C58D93FE997177101F80535A4738CEBCBF389A99B36371EB").unwrap(),
        ); // order how much elements in a group

        // alpha is a generator
        let alpha = BigUint::from_bytes_be(
            &hex::decode("").unwrap(),
        );

        // beta = alpha^i is a generator too
        let beta = alpha.modpow(&ZKP::generate_random_number(&q), &p);
        let zkp = ZKP {
            p: p.clone(),
            q: q.clone(),
            alpha: alpha.clone(),
            beta: beta.clone(),
        };

        let x = ZKP::generate_random_number(&q); // secret
        let k = ZKP::generate_random_number(&q); // key, need to be rand

        let c = ZKP::generate_random_number(&q); // rand value, here is not value but in production will be

        let (y1, y2) = zkp.calculate_two(&x);
        let (r1, r2) = zkp.calculate_two(&k);

        let s = zkp.solve(&k, &c, &x);

        let result = zkp.verify(&r1, &r2, &y1, &y2, &c, &s);
        assert!(result);
    }
}
